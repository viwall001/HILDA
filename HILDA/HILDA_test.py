...
Carbon 14:
    ...
    In the model calculation an arbitrary scale is introduced for 14C such that atmospheric 14C is one unit.
    c14_A:      atmospheric carbon 14                                   = 1
...
class Box():
    def __init__(self, depth, width, width_s=None, c14=0.0):
        self.depth = depth
        self.width = width
...
def c14modelunits_in_deltac14(c14modelunits):
    deltac14 = 1000 * (c14modelunits - 1)
    return deltac14
...
def main():
    ...
    delta = 0.16  # Width of total polar zone; width of low latitude zone = 1-delta
    ...
    c14_A = 1  # in the model calculation an arbitrary scale is introduced for 14C such that atmospheric 14C is one unit
    c14_mean = 0.84  # preindustrial, ocean mean value of carbon 14
    c14_LS = 0.96  # preindustrial delta14C values in range -30 to -50%%
    c14_HS = 0.88  # preindustrial carbon 14 in HSv
    g_LS = 2.32 * 10 ** -7  # derived from (15) with 14C_LS and 14C_HS given (m/s)
    sumCO2 = 2.33  # mol/m**3
    fluxCO2 = 5.41 * 10 ** -7  # mol C/m**2/s; 17.1 mol C/m**2/yr
    a2 = None
    b2 = None
    ...
    def model():
        Atmosphere = Box(12000, 1, None, c14_A)
        I = Box(D, 1 - delta, None, c14_mean)
        LS = Box(hs, 1 - delta, None, c14_LS)
        HD = Box(D, delta, None)
        HS = Box(hs, delta, delta_s, c14_HS)
